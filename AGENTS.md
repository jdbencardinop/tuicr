...
## Working on this fork? Read this first

This repository is `jdbencardinop/tuicr`, an upstream-friendly fork of
[`agavra/tuicr`](https://github.com/agavra/tuicr). Everything below this
notice describes upstream tuicr and still applies unchanged. Before making
fork-specific changes (durable review threads, Azure DevOps/Gitea/Forgejo
adapters, release packaging, `tpatch` customizations), read
**[`docs/fork/AGENT-WORKFLOW.md`](docs/fork/AGENT-WORKFLOW.md)** for the
fork's architecture notes, canonical `ReviewStore` rule, provider
no-silent-loss rules, remote-write approval rule, build/test commands, the
one known pre-existing test failure, the `tpatch` feature lifecycle, and
commit/safety rules (no `--amend`/push/tag unless asked). `CLAUDE.md` at the
repo root just points here.
