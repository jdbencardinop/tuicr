...
| `support-wsl-windows-clipboard` | `8dc5bbd` | `5da2545` | Tracked feature: `state: applied`; recipe replay and preimage verification pass | Live-complete on Ubuntu 24.04/WSL2: direct library, detached-tmux, and real-TUI exports preserve multiline Unicode through Windows Clipboard; injected routing tests isolate native Linux, macOS, and Windows/PowerShell | Fork-only platform integration; upstream candidate if maintainers want first-class WSL behavior |
| `prepare-cross-platform-fork-release` | `828cd12`, `a40c0d2`, `e8f210c` | `0c15cd8` | Tracked feature: `state: applied`; local packaging, Cargo install, and supported library suite pass. Repeated follow-up landings are individually attested, but current `tpatch verify` cannot select one historical baseline across the different generations. | Native Linux x86_64 and both macOS targets pass; Linux arm64 built but its fail-closed scanner finding required the later exact-digest feature | Fork-only release identity, distribution, and promotion policy |
| `allow-arm64-release-scan-noise` | `0aa9427` | `720e4a9` | Tracked feature: `state: applied`; recipe replay and preimage verification pass | Exact-digest-only allowance for a deterministic Linux arm64 compiled-binary match; complete native rerun pending | Fork-only release security policy |
| `defer-nix-release-channel` | `51f42eb` | `e7ccb8f` | Tracked feature: `state: applied`; recipe replay and preimage verification pass | Cargo git install passes; three legacy Nix runs failed on crates.io HTTP 403 responses for changing dependencies, so Nix is explicitly non-release/manual | Fork-only distribution policy |
