...
/// base URL. The server thread is detached; tests are expected to make
...
pub(crate) fn start_mock_server(responses: HashMap<String, (u16, String)>) -> String {
...
fn handle_connection(stream: std::net::TcpStream, responses: &HashMap<String, (u16, String)>) {
    let mut reader = BufReader::new(stream.try_clone().expect("clone mock stream"));
    let mut request_line = String::new();
    if reader.read_line(&mut request_line).unwrap_or(0) == 0 {
        return;
    }
    // Discard headers up to the blank line; every request this backend
    // makes against this mock is a GET with no body.
    loop {
